...
export default function Portfolio({ onEditProfile, onOpenPlans }: { onEditProfile: () => void; onOpenPlans: () => void }) {
  ...
  return <PageTransition><div className="min-h-screen px-5 py-8">
    <div className="mb-6 flex items-center gap-4"><div className="relative h-16 w-16 overflow-hidden rounded-full bg-teal/10">{profile?.isVerified && <motion.div className="absolute inset-0 -translate-x-full" animate={{ x: ["-100%", "200%"] }} transition={{ duration: 2.5, repeat: Infinity, repeatDelay: 3 }} style={{ background: "linear-gradient(120deg, transparent, rgba(232,163,61,0.5), transparent)" }} />}</div><div className="flex-1"><h1 className="text-lg font-bold">{profile?.fullName || "Add your name"}</h1><p className="text-xs text-[var(--text-secondary)]">{profile?.isVerified ? "Verified" : "Not verified"} Â· {profile?.occupation || "Occupation not set"}</p></div><div className="flex flex-col gap-2"><Button variant="secondary" className="!px-3 !py-1.5 !text-xs" onClick={onEditProfile}>Edit</Button><Button variant="secondary" className="!px-3 !py-1.5 !text-xs" onClick={onOpenPlans}>Plan</Button></div></div>
    <div className="mb-6 grid grid-cols-3 gap-3">{loading || !stats ? <><Skeleton className="h-16" /><Skeleton className="h-16" /><Skeleton className="h-16" /></> : <><StatCard value={String(stats.completedJobs)} label="Completed jobs" /><StatCard value={`${stats.repeatClientPct}%`} label="Repeat clients" /><StatCard value={stats.avgRating ? stats.avgRating.toFixed(1) : "â"} label="Avg rating" /></>}</div>
    {!profile?.isVerified && <div className="mb-6 rounded-xl2 border border-[var(--border)] p-4"><p className="mb-2 text-sm font-medium">Get verified â Rs 10,000</p>{verifyError && <p className="mb-2 text-xs text-danger">{verifyError}</p>}<Button loading={verifying} onClick={handlePurchaseVerification}>Purchase Verification</Button></div>}
    ...
    <div className="rounded-xl2 border border-[var(--border)] p-4"><p className="mb-1 text-sm font-medium">Your referral code</p><p className="mb-3 tabular-nums text-lg font-bold text-teal">{referralCode ?? "â"}</p><div className="flex gap-2"><input value={applyCode} onChange={(e) => setApplyCode(e.target.value)} placeholder="Have a code? Enter it" className="flex-1 rounded-xl2 border border-[var(--border)] bg-transparent px-3 py-2 text-xs outline-none focus:border-teal" /><button onClick={handleApplyCode} className="rounded-xl2 bg-teal px-3 py-2 text-xs font-semibold text-white">Apply</button></div>{referralMsg && <p className="mt-2 text-xs text-[var(--text-secondary)]">{referralMsg}</p>}</div>
    {quizCategoryId && <QuizModal open={!!quizCategoryId} onClose={() => setQuizCategoryId(null)} categoryId={quizCategoryId} />}
    <Button variant="destructive" className="mt-6 w-full" onClick={signOut}>Sign Out</Button>
  </div></PageTransition>;
}
