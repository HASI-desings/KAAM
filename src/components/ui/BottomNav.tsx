...
import { Home, Wallet as WalletIcon, Plus, MessageCircle, User, ShieldCheck, Menu } from "lucide-react";

export type NavKey = "feed" | "post" | "wallet" | "chat" | "profile" | "admin";
...
const leftItems: { key: NavKey; label: string; icon: typeof Home }[] = [
  { key: "feed", label: "Feed", icon: Home },
  { key: "wallet", label: "Wallet", icon: WalletIcon }
];
const rightItems: { key: NavKey; label: string; icon: typeof Home }[] = [
...
export function BottomNav({ active, onNavigate, isAdmin }: BottomNavProps) {
  ...
  const right = isAdmin ? [...rightItems, { key: "admin" as NavKey, label: "Admin", icon: ShieldCheck }] : rightItems;

  function renderItem({ key, label, icon: Icon }: { key: NavKey; label: string; icon: typeof Home }) {
          ...
          {expanded ? (
            <motion.div key="expanded" className="flex items-center gap-1" initial="collapsed" animate="expanded" exit="collapsed" variants={containerVariants}>
              ...
              <motion.button
                ...
                className="mx-1 flex h-10 w-10 items-center justify-center rounded-full bg-teal text-white"
                style={{ boxShadow: "0 4px 12px rgba(15,102,89,0.35)" }}
              >
                <Plus size={20} />
              </motion.button>
